...
```
src/lockfile.ts     discoverLockfile(): override path -> platform default; parses LeagueClient:<pid>:<port>:<password>:https
...
src/schemas.ts      response schemas; placeholders until M0.3
src/fixtures.ts     fixture layout, patch naming, top-level shape diff
src/cli/smoke.ts    pnpm --filter @customs/lcu smoke [--diff] [--insecure] [--lockfile p] [--puuid p] [--riot-id N#TAG]
...
Tests never touch a live client: `pnpm --filter @customs/lcu test`.
