...
- [x] **M4.10** (landed 2026-09-10 with M4.7 (a)) The lobby a latecomer can still join: in `filling` and `balanced` the tonight page prints `Missed the invite? The lobby is Customs 09 Sep #1, password 4821.` — the name-only form when no companion has reported a password, and no element at all when the lobby has no name. *(owner: web-engineer; product's words and the designer's placement are in `05-design.md`)*
