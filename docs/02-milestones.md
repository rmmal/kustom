...
- [x] **M5.26** (landed 2026-09-12: `/games` and `/fun` default to Summoner's Rift; a second chip row toggles ARAM via `?queue=aram`. **Amended 2026-09-15:** Howling Abyss customs in this group are `gameMode: KIWI` / `mapId: 12` (queue 3270), not `ARAM`; the ARAM chip lists those. CLASSIC / map 11 / a missing mode are Rift. The rating fold, `/stats`, awards and leaderboard streaks are Rift-only; `/fun` Howling Abyss records stay behind the picker. `/fun` hides CS-by-role on ARAM.) Split Howling Abyss off the game history and the fun records.
